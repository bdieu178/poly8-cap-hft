...
pub struct Strategy {
    ...
    pub gru_weights: Option<GruWeights>,
    pub gru_hidden: [[f32; 128]; 2],
    pub gru_evaluated_this_tick: bool,
    pub last_gru_signed_alpha: f64,
    pub last_gru_nu_probs: [f32; 3],
    pub last_poly_bid: f64,
    pub last_poly_ask: f64,
    pub last_bid_concentration: f64,
    pub last_ask_concentration: f64,
    pub last_whale_imbalance: f64,
    pub last_depth_sweep_cost: f64,
    pub last_rotation_ts: u64,
    pub last_hl_mid: f64,
    pub last_pm_mid: f64,
    pub returns_vol_ema: f64,
    pub rolling_corr_tracker: RollingCorrelationTracker,
    pub ofi_standardizer: OnlineStandardizer,
    pub flow_standardizer: OnlineStandardizer,
    pub path_delta_standardizer: OnlineStandardizer,
    pub path_curvature_standardizer: OnlineStandardizer,
    pub bid_concentration_standardizer: OnlineStandardizer,
    pub ask_concentration_standardizer: OnlineStandardizer,
    pub last_weights_file_mtime: Option<std::time::SystemTime>,
}
...
impl Strategy {
    pub fn new(client: Option<Arc<PolymarketClient>>, db_conn: Option<Arc<Mutex<Connection>>>, is_shadow: bool, asset: String, token_id_up: String, token_id_down: String, strike_price: f64, config: Config, journal: Arc<JournalWriter>, order_tx: flume::Sender<OrderRequest>, telemetry_tx: Option<flume::Sender<serde_json::Value>>) -> Self {
        let weights_path = format!("/home/bdieu178/user/defi-agents/polymarket/models/gru_weights_{}_latest.bin", asset.to_lowercase());
        let mut gru_weights = None;
        let mut last_weights_file_mtime = None;

        #[cfg(not(test))]
        {
            println!("{} [WEIGHTS_INIT] Loading GRU weights from {}...", get_utc_ts(), weights_path);
            match GruWeights::load_from_file(&weights_path) {
                Ok(w) => {
                    println!("{} [WEIGHTS_INIT] GRU weights loaded successfully. Validation stats - AUC: {:.4}, test_auc: {:.4}, Val Brier: {:.4}",
                             get_utc_ts(), w.header.val_auc, w.header.test_auc, w.header.val_brier);
                    if let Ok(metadata) = std::fs::metadata(&weights_path) {
                        if let Ok(mtime) = metadata.modified() {
                            last_weights_file_mtime = Some(mtime);
                        }
                    }
                    gru_weights = Some(w);
                }
                Err(e) => {
                    println!("{} [WEIGHTS_INIT_WARN] Failed to load GRU weights from {}: {}. Falling back to legacy pricing.", get_utc_ts(), weights_path, e);
                }
            }
        }
        ...
        let strategy = Self {
            ...
            gru_weights,
            gru_hidden: [[0.0; 128]; 2],
            gru_evaluated_this_tick: false,
            last_gru_signed_alpha: 0.0,
            last_gru_nu_probs: [0.33, 0.33, 0.34],
            last_poly_bid: 0.5,
            last_poly_ask: 0.5,
            last_bid_concentration: 0.0,
            last_ask_concentration: 0.0,
            last_whale_imbalance: 0.0,
            last_depth_sweep_cost: 0.0,
            last_rotation_ts: 0,
            last_hl_mid: 0.0,
            last_pm_mid: 0.5,
            returns_vol_ema: 0.01,
            rolling_corr_tracker: RollingCorrelationTracker::new(300),
            ofi_standardizer: OnlineStandardizer::new(0.005),
            flow_standardizer: OnlineStandardizer::new(0.005),
            path_delta_standardizer: OnlineStandardizer::new(0.005),
            path_curvature_standardizer: OnlineStandardizer::new(0.005),
            bid_concentration_standardizer: OnlineStandardizer::new(0.005),
            ask_concentration_standardizer: OnlineStandardizer::new(0.005),
            last_weights_file_mtime,
        };
    ...
    pub fn tick(&mut self, l2: &L2BookSnapshot, cog: Option<&CognitionStateStruct>, acc: &AccountStateStruct, pos_info_writer: &mut ShmWriter<PositionInfoStruct>, global_risk_writer: &mut ShmWriter<GlobalRiskStruct>) -> bool {
        ...
        if is_poly_stale || is_halt {
            // Decay GRU hidden state if data is stale to decay stale memory
            for i in 0..2 {
                for j in 0..128 {
                    self.gru_hidden[i][j] *= 0.5;
                }
            }
            let has_open_orders = {
        ...
        if self.ticks_seen > 0 && self.last_hl_mid > 0.0 {
            let pct_return = (hl_mid - self.last_hl_mid) / self.last_hl_mid;
            self.returns_vol_ema = 0.05 * pct_return.abs() + 0.95 * self.returns_vol_ema;
        } else {
            self.returns_vol_ema = 0.01;
        }
        self.last_hl_mid = hl_mid;
        ...
        if l2.poly_up_bids[0].price > 0.0 {
            self.last_poly_bid = l2.poly_up_bids[0].price;
        }
        if l2.poly_up_asks[0].price > 0.0 {
            self.last_poly_ask = l2.poly_up_asks[0].price;
        }
        self.last_bid_concentration = l2.bid_concentration;
        self.last_ask_concentration = l2.ask_concentration;
        self.last_whale_imbalance = if (l2.whale_bid_size + l2.whale_ask_size) > 1e-9 {
            (l2.whale_bid_size - l2.whale_ask_size) / (l2.whale_bid_size + l2.whale_ask_size)
        } else {
            0.0
        };
        let pm_spread_calc = (self.last_poly_ask - self.last_poly_bid).max(0.0);
        self.last_depth_sweep_cost = (pm_spread_calc * 2.0).max(0.01);
        
        self.gru_evaluated_this_tick = false;
        
        if l2.rotation_ts != self.last_rotation_ts {
            println!("{} [ROTATION_CHANGE] Resetting GRU hidden states. Old: {}, New: {}", get_utc_ts(), self.last_rotation_ts, l2.rotation_ts);
            self.gru_hidden = [[0.0f32; 128]; 2];
            self.last_rotation_ts = l2.rotation_ts;
        }

        // Periodic check for weight file hot-swapping
        #[cfg(not(test))]
        {
            if self.ticks_seen % 1000 == 0 {
                let weights_path = format!("/home/bdieu178/user/defi-agents/polymarket/models/gru_weights_{}_latest.bin", self.asset.to_lowercase());
                if let Ok(metadata) = std::fs::metadata(&weights_path) {
                    if let Ok(mtime) = metadata.modified() {
                        if Some(mtime) != self.last_weights_file_mtime {
                            println!("{} [WEIGHTS_RELOAD] File modified. Reloading weights from {}...", get_utc_ts(), weights_path);
                            match GruWeights::load_from_file(&weights_path) {
                                Ok(w) => {
                                    self.gru_weights = Some(w);
                                    self.last_weights_file_mtime = Some(mtime);
                                    self.gru_hidden = [[0.0f32; 128]; 2];
                                    println!("{} [WEIGHTS_RELOAD] Weights reloaded successfully. Resetting GRU hidden states.", get_utc_ts());
                                }
                                Err(e) => {
                                    println!("{} [WEIGHTS_RELOAD_ERR] Failed to reload weights: {}", get_utc_ts(), e);
                                }
                            }
                        }
                    }
        ...
        if now_ms.saturating_sub(self.last_telemetry_ts) >= 100 {
            if let Some(ref tx) = self.telemetry_tx {
                let telemetry = serde_json::json!({
                    ...
                    "gru_active": self.gru_weights.is_some(),
                    "gru_signed_alpha": self.last_gru_signed_alpha,
                    "gru_nu_probs": self.last_gru_nu_probs,
    ...
    fn calculate_base_p_theo(&mut self, hl_mid: f64, ofi_signal: f64, signed_flow: f64, fading_factor: f64, informed_multiplier: f64, path_delta: f64, path_curvature: f64, rotation_ts: u64, pm_spread: f64, max_latency: u64) -> f64 {
        // If GRU weights are loaded, use the GRU-based pricing model
        if self.gru_weights.is_some() {
            if !self.gru_evaluated_this_tick {
                self.run_gru_forward_pass(hl_mid, ofi_signal, signed_flow, path_delta, path_curvature, pm_spread, max_latency);
                self.gru_evaluated_this_tick = true;
            }
            
            let mut baseline_deviation = 0.0;
            if self.strike_price > 0.0 && hl_mid > 0.0 {
            ...
            // ν-Regime Sensitivity Scaling
            let nu_probs = self.last_gru_nu_probs;
            let nu_scaling = if nu_probs[2] > 0.6 {
                // Herding regime: stretch the lag, reduce aggression
                0.7 * informed_multiplier
            } else if nu_probs[0] > 0.6 {
                // Contrarian regime: fast snap, increase aggression
                1.3 * informed_multiplier
            } else {
                informed_multiplier
            };
            
            let scaled_micro = self.last_gru_signed_alpha * nu_scaling * fading_factor;
            let mut final_deviation = baseline_deviation + scaled_micro;
            
            // CLOB Friction Penalties
            // 4a. Longshot Premium Penalty
            if final_deviation > 0.40 || final_deviation < -0.40 {
                let longshot_penalty = pm_spread.max(0.05); // At least 5 cents
                if final_deviation > 0.0 {
                    final_deviation -= longshot_penalty;
                } else {
                    final_deviation += longshot_penalty;
                }
            }
            
            // 4b. Geometric Depth Sweep Discount
            if self.last_depth_sweep_cost > 0.0 {
                let depth_discount = (self.last_depth_sweep_cost / 100.0).clamp(0.0, 0.05); // Max 5 cent discount
                if final_deviation.abs() > 0.10 {
                    if final_deviation > 0.0 {
                        final_deviation -= depth_discount;
                    } else {
                        final_deviation += depth_discount;
                    }
                }
            ...
            return final_deviation.clamp(-0.48, 0.48);
        }
    ...
    fn run_gru_forward_pass(&mut self, hl_mid: f64, ofi_signal: f64, signed_flow: f64, path_delta: f64, path_curvature: f64, pm_spread: f64, max_latency: u64) {
        let weights = match &self.gru_weights {
            Some(w) => w,
            None => return,
        };
        
        let sigma_fast = self.returns_vol_ema;
        let approx_p = if self.strike_price > 0.0 {
            (0.5 + (hl_mid - self.strike_price) / (self.strike_price * sigma_fast.max(0.0001))).clamp(0.01, 0.99)
        } else {
            0.5
        };
        
        let pm_mid = (self.last_poly_bid + self.last_poly_ask) / 2.0;
        let phase_lag = (approx_p - pm_mid).clamp(-0.5, 0.5);
        
        // Update rolling correlation & nu_regime
        let pct_return = if self.last_hl_mid > 0.0 { (hl_mid - self.last_hl_mid) / self.last_hl_mid } else { 0.0 };
        let pm_mid_diff = pm_mid - self.last_pm_mid;
        let corr = self.rolling_corr_tracker.update(pct_return, pm_mid_diff);
        let nu_regime = if corr > 0.3 {
            1.0
        } else if corr < -0.3 {
            -1.0
        } else {
        ...
        let std_ofi = self.ofi_standardizer.update(ofi_signal);
        let std_flow = self.flow_standardizer.update(signed_flow);
        let std_path_delta = self.path_delta_standardizer.update(path_delta);
        let std_path_curv = self.path_curvature_standardizer.update(path_curvature);
        let std_bid_conc = self.bid_concentration_standardizer.update(self.last_bid_concentration);
        let std_ask_conc = self.ask_concentration_standardizer.update(self.last_ask_concentration);
        
        let input_vec: [f32; 15] = [
            std_ofi as f32,
            std_flow as f32,
            std_path_delta as f32,
            std_path_curv as f32,
            sigma_fast as f32,
            approx_p as f32,
            (approx_p * (1.0 - approx_p)) as f32,
            phase_lag as f32,
            nu_regime as f32,
            pm_spread as f32,
            std_bid_conc as f32,
            std_ask_conc as f32,
            self.last_depth_sweep_cost as f32,
            self.last_whale_imbalance as f32,
            if max_latency > 500 { 1.0 } else { 0.0 },
        ];
        
        // Project
        let mut x_proj = [0.0f32; 128];
        linear(128, 15, &weights.proj_weight, &weights.proj_bias, &input_vec, &mut x_proj);
        
        // LayerNorm 0
        let mut x_norm = [0.0f32; 128];
        layer_norm(128, &weights.ln0_weight, &weights.ln0_bias, &x_proj, &mut x_norm);
        
        // GRU Layer 0
        let mut h0_new = [0.0f32; 128];
        gru_step(128, 128, &weights.layer0, &x_norm, &self.gru_hidden[0], &mut h0_new);
        
        // GRU Layer 1
        let mut h1_new = [0.0f32; 128];
        gru_step(128, 128, &weights.layer1, &h0_new, &self.gru_hidden[1], &mut h1_new);
        
        // Update hidden states
        self.gru_hidden[0].copy_from_slice(&h0_new);
        self.gru_hidden[1].copy_from_slice(&h1_new);
        
        // Output LayerNorm
        let mut out_norm = [0.0f32; 128];
        layer_norm(128, &weights.ln_out_weight, &weights.ln_out_bias, &h1_new, &mut out_norm);
        
        // Head 1: signed_alpha
        let mut raw_alpha = [0.0f32; 1];
        linear(1, 128, &weights.head1_weight, &[weights.head1_bias], &out_norm, &mut raw_alpha);
        let signed_alpha = raw_alpha[0].tanh() as f64;
        
        // Head 2: nu_logits
        let mut nu_logits = [0.0f32; 3];
        linear(3, 128, &weights.head2_weight, &weights.head2_bias, &out_norm, &mut nu_logits);
        let mut nu_probs = [0.0f32; 3];
        softmax3(&nu_logits, &mut nu_probs);
        
        // Cache outputs
        self.last_gru_signed_alpha = signed_alpha;
        self.last_gru_nu_probs = nu_probs;
    }
...
// =========================================================================
// GRU MODEL DEFINITIONS & MATH INFERENCE ENGINE
// =========================================================================
...
#[derive(Debug, Clone)]
pub struct GruWeightsHeader {
    pub magic: u32,
    pub version: u32,
    pub input_dim: u32,
    pub hidden_dim: u32,
    pub num_layers: u32,
    pub num_aux_classes: u32,
    pub ts: u64,
    pub val_auc: f32,
    pub test_auc: f32,
    pub val_brier: f32,
...
impl GruWeightsHeader {
    pub fn read<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        ...
        let input_dim = u32::from_le_bytes(buf[8..12].try_into().unwrap());
        let hidden_dim = u32::from_le_bytes(buf[12..16].try_into().unwrap());
        let num_layers = u32::from_le_bytes(buf[16..20].try_into().unwrap());
        let num_aux_classes = u32::from_le_bytes(buf[20..24].try_into().unwrap());
        let ts = u64::from_le_bytes(buf[24..32].try_into().unwrap());
        let val_auc = f32::from_le_bytes(buf[32..36].try_into().unwrap());
        let test_auc = f32::from_le_bytes(buf[36..40].try_into().unwrap());
        let val_brier = f32::from_le_bytes(buf[40..44].try_into().unwrap());
        let nu_accuracy = f32::from_le_bytes(buf[44..48].try_into().unwrap());
        let longshot_fp = f32::from_le_bytes(buf[48..52].try_into().unwrap());
        let mut sha256 = [0u8; 32];
        sha256.copy_from_slice(&buf[52..84]);
...
pub struct GruWeights {
    pub header: GruWeightsHeader,
    pub proj_weight: Vec<f32>,
    pub proj_bias: Vec<f32>,
    pub ln0_weight: Vec<f32>,
    pub ln0_bias: Vec<f32>,
    pub layer0: GruLayerWeights,
    pub layer1: GruLayerWeights,
    pub ln_out_weight: Vec<f32>,
    pub ln_out_bias: Vec<f32>,
    pub head1_weight: Vec<f32>,
    pub head1_bias: f32,
    pub head2_weight: Vec<f32>,
    pub head2_bias: Vec<f32>,
}

impl GruWeights {
    pub fn load_from_file(path: &str) -> Result<Self, String> {
        let mut file = std::fs::File::open(path).map_err(|e| e.to_string())?;
        let header = GruWeightsHeader::read(&mut file).map_err(|e| e.to_string())?;
        
        if header.magic != 0x47525531 {
            return Err(format!("Invalid magic number: 0x{:X}", header.magic));
        }
        if header.version != 2 {
            return Err(format!("Unsupported version: {}", header.version));
        }
        if header.input_dim != 15 || header.hidden_dim != 128 || header.num_layers != 2 || header.num_aux_classes != 3 {
            return Err(format!("Unsupported dimensions: input_dim={}, hidden_dim={}, num_layers={}, num_aux_classes={}",
                header.input_dim, header.hidden_dim, header.num_layers, header.num_aux_classes));
        }
        
        let mut payload_bytes = Vec::new();
        use std::io::Read;
        file.read_to_end(&mut payload_bytes).map_err(|e| e.to_string())?;
        
        // Verify SHA-256 of payload
        let computed_hash = openssl::hash::hash(openssl::hash::MessageDigest::sha256(), &payload_bytes)
            .map_err(|e| e.to_string())?;
        if computed_hash.as_ref() != header.sha256 {
            return Err("SHA-256 hash mismatch of model weights".to_string());
        }
        
        if payload_bytes.len() % 4 != 0 {
            return Err(format!("Payload size {} is not a multiple of 4", payload_bytes.len()));
        }
        
        let float_count = payload_bytes.len() / 4;
        if float_count != 201220 {
            return Err(format!("Expected 201220 floats in payload, found {}", float_count));
        }
        
        let mut floats = vec![0.0f32; float_count];
        for i in 0..float_count {
            let start = i * 4;
            floats[i] = f32::from_le_bytes(payload_bytes[start..start+4].try_into().unwrap());
        }
        
        let mut offset = 0;
        let mut read_floats = |size: usize| -> Vec<f32> {
            let res = floats[offset..offset+size].to_vec();
            offset += size;
            res
        };
        
        let proj_weight = read_floats(128 * 15);
        let proj_bias = read_floats(128);
        let ln0_weight = read_floats(128);
        let ln0_bias = read_floats(128);
        
        let layer0 = GruLayerWeights {
            weight_ih: read_floats(384 * 128),
            weight_hh: read_floats(384 * 128),
            bias_ih: read_floats(384),
            bias_hh: read_floats(384),
        };
        
        let layer1 = GruLayerWeights {
            weight_ih: read_floats(384 * 128),
            weight_hh: read_floats(384 * 128),
            bias_ih: read_floats(384),
            bias_hh: read_floats(384),
        ...
        let ln_out_weight = read_floats(128);
        let ln_out_bias = read_floats(128);
        let head1_weight = read_floats(128);
        let head1_bias = read_floats(1)[0];
        let head2_weight = read_floats(3 * 128);
        let head2_bias = read_floats(3);
        
        assert_eq!(offset, float_count);
...
pub struct PhaseLagTracker {
    pub last_phase_lag: f64,
    pub rolling_avg_lag: f64,
}

impl PhaseLagTracker {
    pub fn new() -> Self {
        Self {
            last_phase_lag: 0.0,
            rolling_avg_lag: 0.0,
    ...
    pub fn update(&mut self, p_approx: f64, pm_mid: f64) {
        let lag = p_approx - pm_mid;
        self.last_phase_lag = lag;
        self.rolling_avg_lag = 0.05 * lag + 0.95 * self.rolling_avg_lag;
    }
}

#[derive(Debug, Clone)]
pub struct RollingCorrelationTracker {
    window_size: usize,
...
impl RollingCorrelationTracker {
    ...
    pub fn update(&mut self, x: f64, y: f64) -> f64 {
        ...
        let denom_x = (self.sum_xx - (self.sum_x * self.sum_x) / n).max(0.0);
        let denom_y = (self.sum_yy - (self.sum_y * self.sum_y) / n).max(0.0);
...
impl OnlineStandardizer {
    ...
    pub fn update(&mut self, val: f64) -> f64 {
        ...
        self.var = (1.0 - self.alpha) * self.var + self.alpha * diff * (val - self.mean);
...
#[inline(always)]
fn linear(out_dim: usize, in_dim: usize, weight: &[f32], bias: &[f32], input: &[f32], output: &mut [f32]) {
    for i in 0..out_dim {
        let offset = i * in_dim;
        let mut sum = bias[i];
        for j in 0..in_dim {
...
#[inline(always)]
fn layer_norm(dim: usize, weight: &[f32], bias: &[f32], input: &[f32], output: &mut [f32]) {
    let mut sum = 0.0;
    for &val in input.iter().take(dim) {
        sum += val;
    }
    ...
    let var = sum_sq / (dim as f32);
    let std = (var + 1e-5).sqrt();
    
    for i in 0..dim {
        output[i] = weight[i] * ((input[i] - mean) / std) + bias[i];
    }
}
...
#[inline(always)]
fn softmax3(input: &[f32; 3], output: &mut [f32; 3]) {
    let max_val = input[0].max(input[1]).max(input[2]);
    let mut sum = 0.0;
    let mut exps = [0.0; 3];
    for i in 0..3 {
        exps[i] = (input[i] - max_val).exp();
        sum += exps[i];
    }
    for i in 0..3 {
        output[i] = exps[i] / sum;
...
fn gru_step(
    ...
    let mut ih_part = [0.0f32; 384];
    let mut hh_part = [0.0f32; 384];
    
    linear(384, in_dim, &weights.weight_ih, &weights.bias_ih, x, &mut ih_part);
    linear(384, 128, &weights.weight_hh, &weights.bias_hh, h, &mut hh_part);
    
    for i in 0..128 {
        let r = sigmoid(ih_part[i] + hh_part[i]);
        let z = sigmoid(ih_part[i + 128] + hh_part[i + 128]);
        let n = (ih_part[i + 256] + r * hh_part[i + 256]).tanh();
...
mod tests {
    ...
    #[tokio::test]
    async fn test_gru_loading_and_inference() {
        let weights_path = "/home/bdieu178/user/defi-agents/polymarket/models/gru_weights_btc_latest.bin";
        if !std::path::Path::new(weights_path).exists() {
            println!("Skipping test_gru_loading_and_inference: latest weights file not found.");
            return;
        }
        
        let loaded = GruWeights::load_from_file(weights_path);
        assert!(loaded.is_ok(), "Failed to load GRU weights: {:?}", loaded.err());
        let w = loaded.unwrap();
        
        assert_eq!(w.header.magic, 0x47525531);
        assert_eq!(w.header.version, 2);
        assert_eq!(w.header.input_dim, 15);
        assert_eq!(w.header.hidden_dim, 128);
        assert_eq!(w.header.num_layers, 2);
        assert_eq!(w.header.num_aux_classes, 3);
        ...
        strategy.gru_weights = Some(w);
        
        let mut l2 = L2BookSnapshot::default();
        l2.hl_bids[0] = PriceLevel { price: 65010.0, size: 10.0 };
        l2.hl_asks[0] = PriceLevel { price: 65020.0, size: 10.0 };
        l2.poly_up_bids[0] = PriceLevel { price: 0.52, size: 100.0 };
        l2.poly_up_asks[0] = PriceLevel { price: 0.54, size: 100.0 };
        l2.rotation_ts = 1700000000;
        
        let mut pos_writer = setup_test_writer("test_gru_pos");
        let mut risk_writer = setup_risk_writer("test_gru_risk");
        let acc = AccountStateStruct::default();
        
        strategy.tick(&l2, None, &acc, &mut pos_writer, &mut risk_writer);
        
        assert!(strategy.gru_evaluated_this_tick);
        assert!(strategy.last_gru_signed_alpha >= -1.0 && strategy.last_gru_signed_alpha <= 1.0);
        
        let probs = strategy.last_gru_nu_probs;
        let probs_sum: f32 = probs.iter().sum();
        assert!((probs_sum - 1.0).abs() < 1e-4, "Softmax probabilities must sum to 1.0, found: {}", probs_sum);
        
        println!("test_gru_loading_and_inference PASSED successfully! Alpha: {}, Nu probs: {:?}", strategy.last_gru_signed_alpha, probs);
    }
}